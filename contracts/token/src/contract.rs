...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Community { recipient, amount } => {
            execute_community(deps, env, info, recipient, amount)
        }
    ...
    for element in investors
    {
     ...
     let sender_addr = deps.api.addr_canonicalize(element.investor.as_str())?;
       ...
       // investor_info.amount_given  += token_amount;
        ...
        investor_info.perday_amount= token_amount / Uint128::new(calculate_total_days as u128);
        investor_info.amount_remain +=token_amount;
        investor_info.first_claim=element.first_claim * decimal_value ;
        investor_info.amount_given+=token_amount;
        config.seed_token_sale -= element.amount_given ;
